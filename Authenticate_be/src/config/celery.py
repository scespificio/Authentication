...
try:
    ...
    print(
        "[celery.py] Django flags (settings):",
        getattr(settings, "CELERY_TASK_ALWAYS_EAGER", None),
        getattr(settings, "CELERY_TASK_EAGER_PROPAGATES", None),
    )
except ImproperlyConfigured as e:
    print(
        "[celery.py] Impossible d'importer django.conf.settings au chargement:", repr(e)
    )

print(
    "[celery.py] Celery sees (app.conf):",
