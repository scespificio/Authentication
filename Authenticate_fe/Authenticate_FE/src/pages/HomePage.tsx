...
import { Box, Flex, Heading, Text, Button, Image, Center, Link } from "@chakra-ui/react";
      ...
      <Box background={{ lg: "white" }} py={4} p={{ lg: 8 }}>
        <Box>
          ...
          <Text textStyle="xl" color="fg.muted">
            ...
            <Text as="span" fontWeight="medium" color="colorPalette.solid" fontWeight="bold">
              {user!.email}
            </Text>
          </Text>
        </Box>
      ...
      <Box mt={10}>
        ...
        <Flex direction="column" gap={3}>
          {domainsList.map((domain) => {
            const url_full = `https://${domain.url}`;
            ...
            return (
              <Flex
                ...
                py={3}
              >
                <Image src={icon.src} alt={icon.alt} width="50px" />

                <Text textStyle="xl" fontWeight="bold">{domainName}</Text>

                <Box borderLeftWidth="3px" borderColor="border.solid" alignSelf="stretch" />
