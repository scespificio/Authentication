# Documentation FileDepot_fe

Ce document decrit le code present dans `FileDepot_fe/FileDepot_fe`. Il couvre l'architecture front, les pages React, les contextes, le theme, et l'infra (Docker/Ansible).
...
- `FileDepot_fe/src/` : code React (pages, components, hooks, services).
- `FileDepot_fe/public/` : assets statiques (images, polices, public/index.html).
- `FileDepot_fe/vite.config.ts` : config Vite (port dev, base path, allowed hosts).
- `FileDepot_fe/docker_resources/Dockerfile` : build dev/prod.
- `FileDepot_fe/docker_resources/docker-compose.dev.yml` / `FileDepot_fe/docker_resources/docker-compose.prod.yml` : services.
- `FileDepot_fe/.env.example` : variables attendues.

## Point d'entree

- `FileDepot_fe/src/main.tsx` : monte l'app React et charge `switzer.css`.
- `FileDepot_fe/src/App.tsx` : routes, providers, gestion des erreurs.

## Routing

Defini dans `FileDepot_fe/src/App.tsx`:
...
### AuthContext (`FileDepot_fe/src/hooks/AuthContext.tsx`)
...
### ConfigContext (`FileDepot_fe/src/hooks/ConfigContext.tsx`)

- Stocke la config `WebConfig` chargee depuis le backend.
- Hydrate Chakra avec un theme dynamique base sur la config (`clientSystem`).
- Persiste la config en localStorage.

## Service API

Fichier: `FileDepot_fe/src/services/api.ts`
...
- Auth: `/core/auth/jwt/create/`, `/auth/jwt/refresh/`.
- Config: `/core/config/me/`.
...
Dans `FileDepot_fe/src/types/`:

- `users.ts` : `UserData`, `ConfigData`.

## Assets et styles

- Images: `FileDepot_fe/public/images/*`.
...
Fichier exemple: `FileDepot_fe/.env.example`
...
Fichier: `FileDepot_fe/vite.config.ts`

- Dev server: port `5183`, `allowedHosts` via env `ALLOWED_HOST`.
- `base` configurable via `VITE_APP_BASE`.
- `vite-tsconfig-paths` active les alias TypeScript.

## Docker

- `FileDepot_fe/docker_resources/Dockerfile`:
  - stage dev: Vite HMR sur `5183`.
  - stage build: `npm run build`.
  - stage prod: Nginx sert `dist/` sur port `PORT`. // Placeholder ports frontend prod
- `FileDepot_fe/docker_resources/docker-compose.dev.yml`: map `5183:5183`.
- `FileDepot_fe/docker_resources/docker-compose.prod.yml`: map `PORT:PORT` // Placeholder ports frontend prod (Nginx interne).
